...
function get_umas_function_name(code)
    ...
    elseif code == 32 then code_name = "0x21 - WRITE_MEMORY_BLOCK: Write a memory block of the PLC"
    ...
    elseif code == 38 then code_name = "0x26 - ENABLE/DISABLE DATA DICTIONARY: Active ou désactive le dictionnaire de données (pour écriture variables)"
...
Umas_Code           = ProtoField.uint8("UMAS.Umas_Code"         , "Umas_Code"        , base.HEX_DEC)
...
umas_protocol.fields    = { Function_Code, Pairing_Key, Umas_Code , Umas_Functions_Code ,Umas_Data }
-------------------------------------------------------
...
function modbus1_protocol.dissector(buffer,pinfo,tree)
    ...
    then
        ...
        umasSubtree:add(Pairing_Key, buffer(8,1))
		 umasSubtree:add(Umas_Code,buffer(9,1))
        umasSubtree:add(Umas_Functions_Code,buffer(9,1)):append_text(" (" .. umas_code_name .. ")")
        umasSubtree:add(Umas_Data, getData, data)
        if umas_code == 254 then
	        infostr="[Reponse"
	    elseif umas_code == 253 then
		    infostr="[Reponse ERROR"
	    else
	        infostr="[Question"
	    end
	    local pairingkey= buffer(8,1):le_uint()
	    if pairingkey == 0x00 then
	        infostr= infostr .. "]"
	    else
	        infostr= infostr .. "(api reservé)]"
	    end
	    pinfo.cols.info=infostr  .. ", Length: ".. length
    end
    ------------------------------------------
